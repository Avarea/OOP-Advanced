...
public class CommandInterpreter : ICommandInterpreter
{
    ...
    public ICommand InterpretCommand(string[] data, string command)
    {
        ...
        object[] injectObjects = fieldsToInject.Select(f => this.serviceProvider.GetService(f.FieldType)).ToArray();

        object[] constructorObjects = new object[] {data}.Concat(injectObjects).ToArray();
